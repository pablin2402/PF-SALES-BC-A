...
export default function ClientInformationOrdenComponent() {
    ...
    const handleToggleTable = () => {
        setShowPayments(!showPayments); // Toggle the visibility of tables
    };
            ...
            <div className="w-full max-w-5xl bg-white p-6 shadow-lg rounded-lg ">
                ...
                <div className="flex mt-4 justify-end space-x-2">
                    ...
                    <button
                        ...
                        className="px-4 py-2 text-lg bg-[#D3423E] text-white font-bold rounded-lg hover:bg-[#FF9C99] hover:text-gray-900 hover:font-bold transition duration-200"
                    >
                        {showPayments ? "Ver productos" : "Ver pagos"}
                    </button>
                </div>
                ...
                {showPayments ? (
                    <div className="mt-5 border border-gray-400 rounded-xl">
                        <table className="w-full text-sm text-left border border-black shadow-xl rounded-2xl overflow-hidden">
                            ...
                            <tbody>
                                {/* Replace this with actual payment data */}
                                {state.payments?.map((payment, index) => (
                                    <tr key={index} className="bg-white border-b hover:bg-gray-50">
                                        <td className="px-6 py-4 text-gray-900">
                                            {new Date(payment.paymentDate).toLocaleDateString("es-ES")}
                                        </td>
                                        <td className="px-6 py-4 text-gray-900">Bs. {payment.amount.toFixed(2)}</td>
                                        <td className="px-6 py-4 text-gray-900">{payment.payerName || "No especificado"}</td>
                                        <td className="px-6 py-4 text-gray-900">
