...
export default function SalesManInformationComponent() {
  ...
  const exportToExcel = async () => {
    try {
      ...
      saveAs(new Blob([buffer], { type: "application/octet-stream" }),
        `Ventas_${client?.fullName || "vendedor"}_${new Date().toISOString().slice(0, 10)}.xlsx`
      );
  ...
  const exportToPDF = async () => {
    try {
      ...
      doc.text(`Fecha: ${new Date().toLocaleDateString("es-ES")}`, 15, 49);
      if (startDate && endDate) {
      ...
      doc.save(`Ventas_${client?.fullName || "vendedor"}_${new Date().toISOString().slice(0, 10)}.pdf`);
      setExportMenuOpen(false);
    } catch (error) {
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  {dateFilterActive ? "Resultados filtrados por fecha" : "Mostrando todos los pedidos"}
                </p>
              </div>
