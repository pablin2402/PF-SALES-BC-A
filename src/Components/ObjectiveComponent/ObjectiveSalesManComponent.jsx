  ...
  <div className="w-full">
    ...
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 mb-6">
      <div className="flex flex-wrap gap-3">
        <SBox className="h-10 w-48 rounded-xl" />
            ...
            <tr>
              {[...Array(11)].map((_, i) => (
                <th key={i} className="px-4 py-3">
                  <SBox className="h-3 w-14" style={{ background: "#d1d5db" }} />
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {[...Array(5)].map((_, i) => (
              <tr key={i} className="border-b border-gray-100" style={{ opacity: 1 - i * 0.15 }}>
                ...
                <td className="px-4 py-4"><SBox className="h-4 w-12" /></td>
                <td className="px-4 py-4"><SBox className="h-4 w-16" /></td>
                <td className="px-4 py-4"><SBox className="h-4 w-14" /></td>
                <td className="px-4 py-4"><SBox className="h-6 w-14 rounded-full" /></td>
                <td className="px-4 py-4">
                  <div className="flex items-center gap-2">
                    <SBox className="w-7 h-7 rounded-full flex-shrink-0" />
                    <SBox className="h-4 w-28" />
                  </div>
                </td>
                <td className="px-4 py-4">
                  <div className="space-y-1.5">
                    <SBox className="h-3 w-10 ml-auto" />
                    <SBox className="h-2 w-36 rounded-full" />
                  </div>
                </td>
              </tr>
            ))}
...
const ObjectiveSalesManComponent = ({ region }) => {
  ...
  const [formData, setFormData] = useState({
    numberOfBoxes: 0, saleLastYear1: 0, startDate: "", endDate: "", categoria: "", ciudad: "", salesMan: "",
  });
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "numberOfBoxes" || name === "saleLastYear1" ? Number(value) : value,
    }));
  };

  const initialFormData = {
    ciudad: "", categoria: "", numberOfBoxes: "", saleLastYear1: "", startDate: "", endDate: "", salesMan: "",
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.status === 200) { fetchObjectiveDataRegion(); setModalOpen(false); setFormData(initialFormData); }
    } catch (err) {
      console.error(err);
  ...
  const fetchSalesmenByIds = async (ids) => {
    ...
    try {
      const res = await axios.post(API_URL + "/whatsapp/salesman/multiple", { ids }, { headers: { Authorization: `Bearer ${token}` } });
      const map = {};
  ...
  const fetchObjectiveDataRegion = async (customFilters = {}) => {
    ...
    try {
      const response = await axios.post(API_URL + "/whatsapp/sales/objective/list", filters, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.post(API_URL + "/whatsapp/category/id", { userId: user, page: 1, id_owner: user, limit: 1000 }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const applyFilters = () => {
    const customFilters = {};
    if (startDate && endDate) { customFilters.startDate = startDate; customFilters.endDate = endDate; }
  ...
  const fetchVendedores = async () => {
    try {
      const response = await axios.post(API_URL + "/whatsapp/sales/list/id", { id_owner: user }, { headers: { Authorization: `Bearer ${token}` } });
      setVendedores(response.data.data);
    } catch (error) { console.error("Error obteniendo vendedores", error); setVendedores([]); }
  };

  useEffect(() => { fetchCategories(); fetchObjectiveDataRegion(); fetchVendedores();     // eslint-disable-next-line react-hooks/exhaustive-deps
}, []);

  const clearFilter = () => {
    setStartDate(""); setEndDate(""); setSelectedPayment(""); setSelectedSaler("");
    setDateFilterActive(false); setPaymentActive(false); setActiveSaler(false);
  ...
  const vendedoresActivos = new Set(objectiveData.map((i) => i.salesManId || i._id).filter(Boolean)).size;
  ...
  return (
    <div className="w-full">
      ...
      {loading ? (
        <SalesManObjectiveSkeleton />
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
            <StatCard icon={FaBullseye}  label="Objetivo total"      value={totalObjetivo.toFixed(0)}        iconBg="#eff6ff" iconColor="#2563eb" />
            <StatCard icon={FaChartLine} label="Vta. año anterior"   value={totalVtaAA.toFixed(0)}           iconBg="#fef3c7" iconColor="#d97706" />
            <StatCard icon={FaBoxOpen}   label="Vta. acumulada"      value={totalCajas.toFixed(0)}           iconBg="#dcfce7" iconColor="#16a34a" />
            <StatCard icon={FaPercent}   label="Avance promedio"     value={avancePromedio.toFixed(1) + "%"} iconBg="#f3e8ff" iconColor="#9333ea" />
            <StatCard icon={FaUsers}     label="Vendedores activos"  value={vendedoresActivos}               iconBg="#fee2e2" iconColor="#D3423E" />
          </div>
          ...
          >
            <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3">
              <div className="flex flex-wrap items-end gap-3 flex-1">
                <div className="flex flex-col gap-1.5 min-w-[180px]">
                  <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Tipo de filtro</label>
                  <select
                ...
                {selectedFilter === "saller" && (
                  <div className="flex flex-col gap-1.5 min-w-[220px]">
                    <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Vendedor</label>
                    <select
                      value={selectedSaler}
              ...
              {isAdmin && (
                <div className="flex gap-2">
                  <button className="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 text-gray-700 text-sm font-bold rounded-xl hover:bg-gray-50 transition active:scale-[0.98]">
            ...
            {(dateFilterActive || paymentFilterActive || saleActive) && (
              <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-gray-100">
                {dateFilterActive && (
                  <span className="inline-flex items-center gap-2 bg-[#D3423E] text-white px-3 py-1 rounded-full text-xs font-bold">
                    {startDate} → {endDate}
                    <button onClick={clearFilter}>×</button>
                  </span>
                )}
                {paymentFilterActive && (
                  <span className="inline-flex items-center gap-2 bg-purple-500 text-white px-3 py-1 rounded-full text-xs font-bold">
                    Pago: {selectedPayment}
                    <button onClick={clearFilter}>×</button>
                  </span>
                )}
                {saleActive && selectedSaler && (
                  <span className="inline-flex items-center gap-2 bg-blue-500 text-white px-3 py-1 rounded-full text-xs font-bold">
          ...
          >
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-200 border-b border-gray-200">
                  <tr>
                    {["Inicio","Fin","Región","Línea","Objetivo","VTA AA","VTA ACUM","VS AA","VS OBJ","Vendedor","Progreso"].map((h, i) => (
                      <th key={i} className="px-4 py-3 text-left text-[11px] font-bold text-gray-600 uppercase tracking-wider whitespace-nowrap">
                        {h}
                      </th>
                    ))}
                  </tr>
                      ...
                      return (
                        <tr key={item.objetivoId || item._id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                          <td className="px-4 py-3 text-gray-700 whitespace-nowrap">
                            {item.startDate ? new Date(item.startDate).toISOString().slice(0,10).split("-").reverse().join("/") : "-"}
                          </td>
                          <td className="px-4 py-3 text-gray-700 whitespace-nowrap">
                            {item.endDate ? new Date(item.endDate).toISOString().slice(0,10).split("-").reverse().join("/") : "-"}
                          </td>
                    ...
                    <tr>
                      <td colSpan={11} className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center justify-center text-gray-400">
                          ...
                          <p className="text-base font-semibold text-gray-700">No se encontraron coincidencias</p>
                ...
                {objectiveData.length > 0 && (
                  <tfoot>
                    <tr className="bg-gray-50 font-bold text-gray-900 border-t-2 border-gray-200">
                      ...
                      <td className="px-4 py-3">
                        {(objectiveData.reduce((s, i) => s + ((i.caja / i.saleLastYear) * 100 || 0), 0) / objectiveData.length).toFixed(2)}%
                      </td>
                      <td className="px-4 py-3">
          ...
          >
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
                ...
                <button onClick={() => setModalOpen(false)} className="text-gray-400 hover:text-gray-700 text-2xl leading-none transition">×</button>
              </div>
              ...
              <div className="p-6 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {[
                  { label: "Número de cajas",  name: "numberOfBoxes", type: "number" },
                  { label: "Venta año pasado", name: "saleLastYear1",  type: "number" },
              ...
              <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex gap-3 shrink-0">
                ...
                <button
                  ...
                  disabled={!formData.categoria || !formData.ciudad || !formData.numberOfBoxes || !formData.saleLastYear1 || !formData.startDate || !formData.endDate}
                  className={`flex-1 px-4 py-2.5 text-sm font-bold uppercase rounded-xl transition active:scale-[0.98] ${
                    !formData.categoria || !formData.ciudad || !formData.numberOfBoxes || !formData.saleLastYear1 || !formData.startDate || !formData.endDate
        ...
        {showObjectiveErrorModal && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-xl font-bold text-gray-900 mb-1">Error al crear el objetivo</h2>
